...
class UnistylesState {
  ...
  initSettings = settings => {
    ...
    if (settings?.initialTheme && settings.adaptiveThemes) {
      throw (0, _utils.error)('You\'re trying to set initial theme and enable adaptiveThemes, but these options are mutually exclusive.');
    }

    // Adaptive themes
    if (settings?.adaptiveThemes) {
      if (!this.themes.get('light') || !this.themes.get('dark')) {
        throw (0, _utils.error)(`You're trying to enable adaptiveThemes, but you didn't register both 'light' and 'dark' themes.`);
      }
      this.themeName = (0, _utils.schemeToTheme)(this.services.runtime.colorScheme);
      return;
    }
    if (settings?.initialTheme) {
      ...
      if (!this.themes.get(initialTheme)) {
        throw (0, _utils.error)(`You're trying to select theme "${initialTheme}" but it wasn't registered.`);
      }
  ...
  initBreakpoints = breakpoints => {
    ...
    if (breakpointsEntries.length === 0) {
      throw (0, _utils.error)('StyleSheet.configure\'s breakpoints can\'t be empty.');
    }
    if (breakpointsEntries?.[0]?.[1] !== 0) {
      throw (0, _utils.error)('StyleSheet.configure\'s first breakpoint must start from 0.');
    }
    breakpointsEntries.sort(([, a], [, b]) => a - b).forEach(([breakpoint, value]) => {
      ...
      mediaQuery.addEventListener('change', event => {
        ...
        this.services.listener.emitChange(_NativePlatform.UnistyleDependency.Breakpoints);
      });
    });
  };
  getConfig = () => this._config;
}
exports.UnistylesState = UnistylesState;
