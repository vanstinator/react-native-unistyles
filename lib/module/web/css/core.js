...
export const convertToCSS = (hash, value, state, containerName) => {
  Object.entries(value).forEach(([styleKey, styleValue]) => {
      ...
      Object.entries(styleValue).forEach(([pseudoStyleKey, pseudoStyleValue]) => {
        if (typeof pseudoStyleValue === 'object' && pseudoStyleValue !== null) {
          ...
          Object.entries(pseudoStyleValue).forEach(([breakpointStyleKey, breakpointStyleValue]) => {
            const query = containerName ? getContainerQuery(pseudoStyleKey, allBreakpoints, containerName) : getMediaQuery(pseudoStyleKey, allBreakpoints);
            state.set({
              mediaQuery: query,
    ...
    if (typeof styleValue === 'object') {
      Object.entries(styleValue).forEach(([breakpointStyleKey, breakpointStyleValue]) => {
        ...
        const query = containerName ? getContainerQuery(styleKey, allBreakpoints, containerName) : getMediaQuery(styleKey, allBreakpoints);
        state.set({
          mediaQuery: query,
          className: hash,
          propertyKey: breakpointStyleKey,
